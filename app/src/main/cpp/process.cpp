...
typedef unsigned short char16_ust;
...
public:
    ...
    void set(const char16_ust *o, size_t numChars) {
        if (mString) {
            free(mString);
        }
...
Java_com_stephen_redfindemo_feature_terminal_TermExec_sendSignal(JNIEnv *env, jclass clazz,
                                                                 jint procId, jint signal) {
    kill(procId, signal);
}

JNIEXPORT jint JNICALL
Java_com_stephen_redfindemo_feature_terminal_TermExec_waitFor(JNIEnv *env, jclass clazz,
                                                              jint procId) {
    int status;
...
JNIEXPORT jint JNICALL
Java_com_stephen_redfindemo_feature_terminal_TermExec_createSubprocessInternal(JNIEnv *env, jclass clazz,
